...
//
use shared::{core::requests::DATE_AT_BOOT_REQUEST, debug};
...
{
    //
    // Main should handle the check if the request is valid,
    // this makes this function look cleaner..
    // So we don't need to check if it's valid here
    //
    let response = DATE_AT_BOOT_REQUEST.response().unwrap();
